...
try:
    # Try different import strategies to be more robust
    try:
        # First try direct import
        from shared_client import start_client
        logger.info("Successfully imported from shared_client")
    except ImportError:
        # Fall back to project-namespaced import
        try:
            from imforware.shared_client import start_client
            logger.info("Successfully imported from imforware.shared_client")
        except ImportError:
            # Last resort: try adjusting path and importing
            module_dir = os.path.dirname(os.path.abspath(__file__))
            if module_dir not in sys.path:
                sys.path.insert(0, module_dir)
            logger.info(f"Adjusted path to include: {module_dir}")
            from shared_client import start_client
            logger.info("Successfully imported from adjusted path")
